...
int Kinematic::Run(double energy, double*& results) {
	...
	//calculate range
	range = compound_range(fProjectile, fTarget, energy, -7);

	//calculate dE/dx
#ifdef debug_irma
	std::cout<<"calling compound_stopping_power("<<fProjectile->Name()<<", "<<fTarget->Name()<<", "<<energy<<")"<<std::endl;
#endif

	dE_dx = compound_stopping_power(fProjectile, fTarget, energy);

	//calculate energy loss
	en_after_target = energy_loss(fProjectile, fTarget, fTargetThickness, energy, -4);
	...
	dE_dx_after_target = compound_stopping_power(fProjectile, fTarget, en_after_target);

	//calculate energy straggling
	en_straggling = energy_straggling(fProjectile, fTarget, dE_dx, dE_dx_after_target, en_loss);

	//calculate angular straggling
	ang_straggling = angular_straggling(fProjectile, fTarget, fTargetThickness, energy);

	//calculate average charge state
	chargestate = charge_state(fProjectile, energy);
...
TSpline3* Kinematic::RangeVsEnergy(double maximum_energy, double step_size) {
	...
	for(i=0;i<NumberOfSteps;i++) {
		en = i*step_size;
		energy[i] = en;
		range[i] = compound_range(fProjectile, fTarget, en, -7);
#ifdef debug_irma
		if(write_file > 0)
...
TSpline3* Kinematic::EnergyVsThickness(double beam_energy, double step_size) {
	...
	for(i=0;i<NumberOfSteps;i++) {
		thick = i*step_size;
		thickness[i] = thick;
		energy[i] = energy_loss(fProjectile, fTarget, thick, beam_energy, -4);
#ifdef debug_VsX
...
TSpline3* Kinematic::RutherfordVsThickness(double beam_energy, double step_size, bool normalize) {
	...
	for(i=0;i<NumberOfSteps;i++) {
		thick = i*step_size;
		thickness[i] = thick;
		en = energy_loss(fProjectile, fTarget, thick, beam_energy, -4);
#ifdef debug_vsx
...
double Kinematic::Range(double energy) {
	return compound_range(fProjectile, fTarget, energy, -7);
}

double Kinematic::EnergyLoss(double energy) {
	return (energy - energy_loss(fProjectile, fTarget, fTargetThickness, energy, -4));
}

double Kinematic::EnergyAfterTarget(double energy) {
	return energy_loss(fProjectile, fTarget, fTargetThickness, energy, -4);
}
...
double Kinematic::Energy(double range, double max_energy, double tolerance) {
	...
	tmp_range = compound_range(fProjectile, fTarget, tmp_energy, -7);
