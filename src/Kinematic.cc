...
Kinematic::Kinematic(Element* pro, Material* tar)
{
  ...
  target = tar;
#ifdef debug_irma
  std::cout<<"Created instance of class Kinematic with projectile: "<<projectile->A()<<projectile->Name()<<std::endl
      <<"and target "<<target->Name()<<std::endl;
  for(int i=0;i<target->NumberOfElements();i++)
    {
      std::cout<<"    "<<i<<".: "<<target->A(i)<<target->Name(i)<<" ("<<target->Fraction(i)<<")"<<std::endl;
    }
#endif
}
...
Kinematic::Kinematic(Element* pro, Material* tar, double target_thick)
{
  projectile = pro;
  target = tar;
  target_thickness = target_thick;
#ifdef debug_irma
  std::cout<<"Created instance of class Kinematic with projectile: "<<projectile->A()<<projectile->Name()<<std::endl
      <<"and target "<<target->Name()<<" (thickness: "<<target_thick<<")"<<std::endl;
  for(int i=0;i<target->NumberOfElements();i++)
    {
      std::cout<<"    "<<i<<".: "<<target->A(i)<<target->Name(i)<<" ("<<target->Fraction(i)<<")"<<std::endl;
    }
...
#ifdef debug_irma
  std::cout<<"Kinematic::run: "<<energy<<", "<<target_thickness<<std::endl;
#endif

//  if(!(target_thickness > 0) && !(target_thickness < 0))
//    {
//      std::cout<<"ERROR, target_thickness ("<<target_thickness<<") is not set!"<<std::endl;
//
//      return 1;
//    }

  //calculate range
  range = compound_range(projectile, target, energy, -7);

  //calculate dE/dx
#ifdef debug_irma
  std::cout<<"calling compound_stopping_power("<<projectile->Name()<<", "<<target->Name()<<", "<<energy<<")"<<std::endl;
#endif

  dE_dx = compound_stopping_power(projectile, target, energy);

  //calculate energy loss
  en_after_target = energy_loss(projectile, target, target_thickness, energy, -4);
  ...
  dE_dx_after_target = compound_stopping_power(projectile, target, en_after_target);

  //calculate energy straggling
  en_straggling = energy_straggling(projectile, target, dE_dx, dE_dx_after_target, en_loss);

  //calculate angular straggling
  ang_straggling = angular_straggling(projectile, target, target_thickness, energy);

  //calculate average charge state
  chargestate = charge_state(projectile, energy);
...
TSpline3* Kinematic::RangeVsEnergy(double maximum_energy, double step_size)
{
  ...
  char tmpname[256];
  int write_file = snprintf(tmpname,256,"RangeVsEnergy_%d_%d.dat",projectile->N(),projectile->Z());
  std::ofstream tmp;
  if(write_file < 0 || write_file >= 256)
    {
      std::cerr<<"Error, string for filename too long, skipping writing to RangeVsEnergy_"<<projectile->N()<<"_"<<projectile->Z()<<".dat (write_file = "<<write_file<<")"<<std::endl;
      write_file = -1;
    }
  else
    tmp.open(tmpname);
  ...
  for(i=0;i<NumberOfSteps;i++)
    {
      en = i*step_size;
      energy[i] = en;
      range[i] = compound_range(projectile, target, en, -7);
#ifdef debug_irma
      if(write_file > 0)
...
TSpline3* Kinematic::EnergyVsRange(double maximum_energy, double step_size)
{
  ...
  char tmpname[256];
  int write_file = snprintf(tmpname,256,"EnergyVsRange_%d_%d.dat",projectile->N(),projectile->Z());
  std::ofstream tmp;
  if(write_file < 0 || write_file >= 256)
    {
      std::cerr<<"Error, string for filename too long, skipping writing to EnergyVsRange_"<<projectile->N()<<"_"<<projectile->Z()<<".dat (write_file = "<<write_file<<")"<<std::endl;
      write_file = -1;
    }
  else
    tmp.open(tmpname);
...
//    }
  ...
  //declaring needed variables
  int NumberOfSteps = (int)(target_thickness/step_size)+1;
  double* thickness;
  double* energy;
  if(NumberOfSteps > 1)
    {
  ...
  for(i=0;i<NumberOfSteps;i++)
    {
      thick = i*step_size;
      thickness[i] = thick;
      energy[i] = energy_loss(projectile, target, thick, beam_energy, -4);
#ifdef debug_VsX
...
#ifdef debug_VsX
  std::cout<<"EnergyVsThickness: done with calculation of energy loss in "<<target_thickness<<" mg/cm2 and "<<(int)(target_thickness/step_size)<<" steps"<<std::endl;
#endif

  /* create TSpline3 object */
  //g      = new TGraph((int)(target_thickness/step_size), thickness, energy);
...
//  if(!(target_thickness > 0) && !(target_thickness < 0))
//    {
//      std::cerr<<"ERROR, target_thickness ("<<target_thickness<<") is not set!"<<std::endl;
//
//      return NULL;
//    }
  if(16*step_size > target_thickness && normalize)
    {
      std::cerr<<"Error, step_size is to big to normalize, setting step_size to "<<target_thickness/16.<<std::endl;
      step_size = target_thickness/16.;
    }
...
#ifdef debug
  std::cout<<"allocating thickness and rutherford as double arrays of length "<<(int)(target_thickness/step_size)+1<<flush;
#endif
  ...
  for(i=0;i<NumberOfSteps;i++)
    {
      thick = i*step_size;
      thickness[i] = thick;
      en = energy_loss(projectile, target, thick, beam_energy, -4);
#ifdef debug_vsx
...
TSpline3* Kinematic::EnergyLossVsEnergy(double max_energy, double step_size, double factor)
{
  ...
  std::cout<<"projectile = "<<projectile->Name()<<", A = "<<projectile->A()<<", N = "<<projectile->N()<<", Z = "<<projectile->Z()<<std::endl
      <<"target = "<<target->Name()<<", thickness = "<<target_thickness<<std::endl;
#endif
...
#ifdef debug_VsE
  std::cout<<"EnergyLossVsEnergy: done with calculation of energy loss in "<<target_thickness<<" mg/cm2 and "<<NumberOfSteps<<" steps"<<std::endl;
#endif

  /* create TSpline3 object */
  spline = new TSpline3(Form("EnergyLossVsEnergy_%s_%s",projectile->Name(),target->Name()), energy, energyloss, NumberOfSteps);
...
#ifdef debug_VsE
  std::cout<<"EnergyLossVsEnergy: done with calculation of energy loss in "<<target_thickness<<" mg/cm2 and "<<(int)(max_energy/step_size)<<" steps"<<std::endl;
#endif
...
{
  return compound_range(projectile, target, energy, -7);
}

double Kinematic::EnergyLoss(double energy)
{
  return (energy - energy_loss(projectile, target, target_thickness, energy, -4));
}

double Kinematic::EnergyAfterTarget(double energy)
{
  return energy_loss(projectile, target, target_thickness, energy, -4);
}
...
double Kinematic::Energy(double range, double max_energy, double tolerance)
{
  ...
  tmp_range = compound_range(projectile, target, tmp_energy, -7);
...
double Kinematic::compound_range(Element* projectile, Material* target, double energy, int integration_limit)
{
#ifdef debug_irma
  std::cout<<"starting compound_range("<<flush<<projectile->A()<<projectile->Name()<<", "<<flush<<target->Name()<<", "<<energy<<", "<<integration_limit<<")"<<std::endl;
#endif
  ...
  for(int i=0;i<n;i++)
    {
      h = (en_2 - en_1)/6.;
      range += h*(0.3/compound_stopping_power(projectile, target, en_1)       + 1.5/compound_stopping_power(projectile, target, en_1 + h)
		+ 0.3/compound_stopping_power(projectile, target, en_1 + 2*h) + 1.8/compound_stopping_power(projectile, target, en_1 + 3*h)
		+ 0.3/compound_stopping_power(projectile, target, en_2 - 2*h) + 1.5/compound_stopping_power(projectile, target, en_2 - h)
		+ 0.3/compound_stopping_power(projectile, target, en_2));
      en_1 = en_2;
      en_2 *= 10;
#ifdef debug_irma
...
#ifdef debug_irma
  std::cout<<"Done with loop, returning compound_range = "<<range<<std::endl;
#endif

  return range;
}

double Kinematic::compound_stopping_power(Element* projectile, Material* target, double energy)
{
#ifdef debug_irma
  std::cout<<"starting compound_stopping_power("<<flush<<projectile->A()<<projectile->Name()<<", "<<flush<<target->Name()<<", "<<energy<<")"<<std::endl;
#endif
  double res;
  ...
  for(int i=0;i<target->NumberOfElements();i++)
    {
      res += target->Fraction(i)*stopping_power(projectile, target->GetElement(i), energy, target->Gaseous());
#ifdef debug_results
...
double Kinematic::stopping_power(Element* projectile, Element* target, double energy, bool gaseous)
{
#ifdef debug_irma
  std::cout<<"starting stopping_power("<<projectile->A()<<projectile->Name()<<", "<<target->A()<<target->Name()<<", "<<energy<<", "<<gaseous<<")"<<std::endl;
#endif
  int z_p = projectile->Z();
  double a_p = projectile->A();
  int z_t = target->Z();
  ...
  if(z_p != 2)
    {
      ...
      // old version with strips (the functions are not continous)
      /*
#ifdef debug_results
      std::cout<<"STOP4: "<<e_h;
#endif
...
#ifdef debug_results
	  std::cout<<" "<<a_h(1,z_t)<<" "<<sqrt(e_h)<<" "<<dedx_e<<std::endl;
#endif
	}
      else
	{
...
double Kinematic::energy_loss(Element* projectile, Material* target, double target_thickness, double energy, int integration_limit)
{
#ifdef debug_irma
  std::cout<<"started energy_loss("<<projectile->A()<<projectile->Name()<<", "<<target->Name()<<", "<<target_thickness<<", "<<energy<<", "<<integration_limit<<")"<<std::endl;
#endif
  double en;
  double range;
  double dedx;
  ...
  if(target_thickness < 0)
    {
      std::cout<<"Warning, target thickness < 0: energy_loss("<<projectile->A()<<projectile->Name()<<", "<<target->Name()<<", "<<target_thickness<<", "<<energy<<", "<<integration_limit<<"), returning input energy"<<std::endl;
      return energy;
    }
  ...
  if(energy <= 0)
    ...
    {
      range = compound_range(projectile, target, energy, integration_limit);
      dedx = compound_stopping_power(projectile, target, energy);
    }
...
#endif
      x += dx;
      dedx = compound_stopping_power(projectile, target, en);
    }
}

double Kinematic::energy_straggling(Element* projectile, Material* target, double dE_dx, double dE_dx_after_target, double energy_loss)
{
...
double Kinematic::angular_straggling(Element* projectile, Material* target, double target_thickness, double energy)
{
...
double Kinematic::charge_state(Element* projectile, double energy)
{
...
double Kinematic::shell_correction(int z)
{
...
double Kinematic::a_h(int index, int z)
{
...
double Kinematic::a_he(int index, int z)
{
...
double Kinematic::b_he(int index, int z)
{
